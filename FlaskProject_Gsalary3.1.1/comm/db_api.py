...
def query_field_from_table(table_name, field_name,where_conditions):
    conn = create_db_connection()
    if conn is None:
        logger.error("无法建立数据库连接，程序退出。")
        return []
    ...
    try:
        ...
        for column, value in where_conditions.items():
            if isinstance(value, list):
                ...
                where_clause.append(f"{column} IN ({placeholders})")
                values.extend(value)
            else:
                where_clause.append(f"{column} = %s")
                values.append(value)
        ...
        if where_str:
            sql = f"SELECT {field_name} FROM {table_name} WHERE {where_str}"
        else:
...
def query_database(table_name, where_conditions):
    conn = create_db_connection()
    if conn is None:
        logger.error("无法建立数据库连接，程序退出。")
        return []
